...
class RedzoneView(View):
    ...
    @discord.ui.button(label="Join Redzone", style=discord.ButtonStyle.primary)
    async def join(self, interaction: discord.Interaction, button: Button):
        self.joined_users.add(interaction.user.id)
        await interaction.response.send_message("You've joined this redzone!", ephemeral=True)

    async def start_outcome_prompt(self, ctx):
        ...
        class OutcomeView(View):
            ...
            async def win(self, interaction: discord.Interaction, button: Button):
                split = WIN_AMOUNT // len(self.view.joined_users) if self.view.joined_users else 0
        ...
        await ctx.send("⏳ Redzone over. Was it a win or a loss?", view=OutcomeView())

@bot.event
async def on_ready():
    print(f"Logged in as {bot.user.name}")
    await update_leaderboard(await bot.fetch_guilds().flatten()[0])
...
async def start(ctx):
    ...
    view = RedzoneView()
    await ctx.send("🚨 Redzone Started! Click below to join. You have 6 minutes.", view=view)
    await view.start_outcome_prompt(ctx)
